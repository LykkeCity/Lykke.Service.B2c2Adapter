...
namespace Lykke.Service.B2c2Adapter.Services
{
    public class OrderBooksService : IStartable, IStopable
    {
        ...
        private void InitializeAssetPairs()
        {
            ...
            _log.Debug("Started instrument initialization.");
            ...
            _log.Debug("Finished instrument initialization.", new { instruments.Count });
        }
        ...
        private async Task ReconnectIfNeeded(ITimerTrigger timer, TimerTriggeredHandlerArgs args, CancellationToken ct)
        {
            ...
            {
                _log.Error(e, "Error during ReconnectIfNeeded.");
            }
        }
        ...
        private void ReconnectIfNeeded()
        {
            ...
            if (needToReconnect)
            {
                ...
                _log.Debug("Need to reconnect.", new
                {
                    oldest?.Asset,
                    secondsPassed,
                    hasAny,
        ...
        private void ForceReconnect()
        {
            ...
            lock (_syncReconnect)
            {
                ...
                foreach (var instrumentLevels in _instrumentsLevels)
                {
                    ...
                    _b2C2WebSocketClient.SubscribeAsync(instrumentWithSuffix, levels, HandleAsync)
                        ...
                        {
                            if (x.Exception != null)
                                _log.Debug("Exception while subscribing to an instrument.", exception: x.Exception.InnerException, context: new { instrument });
                            else
            ...
            _log.Debug("Finished subscribing.");
        }
        ...
        private async Task PublishOrderBookAndTickPrice(OrderBook orderBook, string assetPairWithSlash)
        {
            ...
            {
                _log.Debug("Stale instrument.", new { orderBook.Asset });
